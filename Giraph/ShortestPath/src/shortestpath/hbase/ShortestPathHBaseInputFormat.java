...
public class ShortestPathHBaseInputFormat extends
	...
	public class LongDoubleDoubleHBaseVertexReader
		...
		public Vertex<LongWritable, DoubleWritable, DoubleWritable> getCurrentVertex()
			...
			DoubleWritable vertexValue = new DoubleWritable(Bytes.toDouble(row
					.getValue(Bytes.toBytes("Value"), Bytes.toBytes("col1"))));

			ByteArrayInputStream byteIn = new ByteArrayInputStream(row.getValue(
					Bytes.toBytes("outEdges"), Bytes.toBytes("col2")));
			ObjectInputStream in = new ObjectInputStream(byteIn);
			Map<Long, Double> outEdges = null;
			try {
				outEdges = (Map<Long, Double>) in.readObject();
			} catch (ClassNotFoundException e) {
				System.out.println("Vertex : ClassNotFoundException");
				System.exit(-1);
			}
