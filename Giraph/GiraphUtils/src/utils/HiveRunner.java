...
public class HiveRunner implements Tool {

	/** logger */
	private static final Logger LOG = Logger.getLogger(HiveRunner.class);
	...
	private List<VertexInputFormatDescription> vertexInputDescriptions = Lists
			.newArrayList();

	/** Descriptions of edge input formats */
	private List<EdgeInputFormatDescription> edgeInputDescriptions = Lists
			.newArrayList();
	 ...
	 * Add vertex input
	 * 
	 * @param hiveToVertexClass
	 *          HiveToVertex class to use
	 * @param tableName
	 *          Table name
	 * @param partitionFilter
	 *          Partition filter, or null if no filter used
	 * @param additionalOptions
	 *          Additional options, in the form "option=value"
	 */
	public void addVertexInput(Class<? extends HiveToVertex> hiveToVertexClass,
		...
		VertexInputFormatDescription description = new VertexInputFormatDescription(
				HiveVertexInputFormat.class);
		description.addParameter(HIVE_VERTEX_INPUT.getClassOpt().getKey(),
				hiveToVertexClass.getName());
		description.addParameter(HIVE_VERTEX_INPUT.getProfileIdOpt().getKey(),
				"vertex_input_profile_" + vertexInputDescriptions.size());
		description.addParameter(HIVE_VERTEX_INPUT.getTableOpt().getKey(),
				tableName);
		if (partitionFilter != null && !partitionFilter.isEmpty()) {
			description.addParameter(HIVE_VERTEX_INPUT.getPartitionOpt().getKey(),
					partitionFilter);
		}
	 ...
	 * Add edge input
	 * 
	 * @param hiveToEdgeClass
	 *          HiveToEdge class to use
	 * @param tableName
	 *          Table name
	 * @param partitionFilter
	 *          Partition filter, or null if no filter used
	...
	public void addEdgeInput(Class<? extends HiveToEdge> hiveToEdgeClass,
			String tableName, String partitionFilter, String... additionalOptions) {
		EdgeInputFormatDescription description = new EdgeInputFormatDescription(
				HiveEdgeInputFormat.class);
		description.addParameter(HIVE_EDGE_INPUT.getClassOpt().getKey(),
				hiveToEdgeClass.getName());
		description.addParameter(HIVE_EDGE_INPUT.getProfileIdOpt().getKey(),
				"edge_input_profile_" + edgeInputDescriptions.size());
		description.addParameter(HIVE_EDGE_INPUT.getTableOpt().getKey(), tableName);
		if (partitionFilter != null && !partitionFilter.isEmpty()) {
			description.addParameter(HIVE_EDGE_INPUT.getPartitionOpt().getKey(),
					partitionFilter);
		}
		addAdditionalOptions(description, additionalOptions);
	 ...
	 * Set vertex output
	 * 
	 * @param vertexToHiveClass
	 *          class for writing vertices to Hive.
	 * @param tableName
	 *          Table name
	 * @param partitionFilter
	 *          Partition filter, or null if no filter used
	 */
	public void setVertexOutput(Class<? extends VertexToHive> vertexToHiveClass,
			String tableName, String partitionFilter) {
	 ...
	 * 
	 * @param conf
	 *          Configuration to create ImmutableClassesGiraphConfiguration from
	 *          and update with any changes to the returned configuration
	 ...
	 *          InputFormatDescription to update with any changes to the returned
	 *          configuration
	 * @return ImmutableClassesGiraphConfiguration
	 */
	private ImmutableClassesGiraphConfiguration createGiraphConf(
			Configuration conf, final InputFormatDescription inputFormatDescription) {
		return new ImmutableClassesGiraphConfiguration(conf) {
	 ...
	 * the metastore and fail earlier in the case that metadata doesn't exist. In
	 * the case of multiple vertex input descriptions, metadata is cached in each
	 * vertex input format description and then saved into a single Configuration
	 * via JSON.
	 */
	...
	public void prepareHiveVertexInputs() {
		if (vertexInputDescriptions.size() == 1) {
			...
			createGiraphConf(conf).createWrappedVertexInputFormat().checkInputSpecs(
					conf);
		} else {
			...
			for (int i = 0; i < vertexInputDescriptions.size(); i++) {
				...
				final VertexInputFormatDescription vertexInputDescription = vertexInputDescriptions
						.get(i);
	 ...
	 * metadata information into the configuration to eliminate worker access to
	 * the metastore and fail earlier in the case that metadata doesn't exist. In
	 * the case of multiple edge input descriptions, metadata is cached in each
	 * vertex input format description and then saved into a single Configuration
	 * via JSON.
	 */
	@SuppressWarnings("unchecked")
	public void prepareHiveEdgeInputs() {
		if (edgeInputDescriptions.size() == 1) {
			GiraphConstants.EDGE_INPUT_FORMAT_CLASS.set(conf, edgeInputDescriptions
					.get(0).getInputFormatClass());
			...
			// data from metastore, and check it
			createGiraphConf(conf).createWrappedEdgeInputFormat().checkInputSpecs(
					conf);
		} else {
			...
			for (int i = 0; i < edgeInputDescriptions.size(); i++) {
				...
				// original one
				Configuration confCopy = new Configuration(conf);
				final EdgeInputFormatDescription edgeInputDescription = edgeInputDescriptions
						.get(i);
	...
	private CommandLine handleCommandLine(String[] args) throws ParseException,
		...
		if (computationClass == null) {
			throw new IllegalArgumentException("Need the Giraph "
					+ Computation.class.getSimpleName()
		...
		if (edgeInputs != null && edgeInputs.length != 0) {
			...
			for (String edgeInput : edgeInputs) {
				...
				addEdgeInput(findClass(parameters[0], HiveToEdge.class), parameters[1],
						elementOrNull(parameters, 2), copyOfArray(parameters, 3));
			}
		}
		...
		if (output != null) {
			...
			if (parameters.length < 2) {
				throw new IllegalStateException("Illegal output description " + output
						+ " - VertexToHive class and table name needed");
			}
		...
		if (!hasVertexInput() && !hasEdgeInput()) {
			throw new IllegalArgumentException("Need at least one of Giraph "
					+ HiveToVertex.class.getSimpleName() + " (-vertexInput) and "
					+ HiveToEdge.class.getSimpleName() + " (-edgeInput)");
		}
		if (vertexToHiveClass == null && !skipOutput) {
			throw new IllegalArgumentException("Need the Giraph "
					+ VertexToHive.class.getSimpleName() + " (-output) to use");
		}
	...
	private void addOptions(Options options) {
		...
		options
				.addOption(
						...
						"vertexInput",
						true,
						getInputOptionDescription("vertex",
								HiveToVertex.class.getSimpleName()));

		// Edge input settings
		options.addOption("ei", "edgeInput", true,
				getInputOptionDescription("edge", HiveToEdge.class.getSimpleName()));
	 ...
	 * 
	 * @param inputType
	 *          Type of input (vertex or edge)
	 * @param hiveToObjectClassName
	 *          HiveToVertex or HiveToEdge
	...
	private static String getInputOptionDescription(String inputType,
		...
		inputOption.append("Giraph ").append(hiveToObjectClassName)
				.append(" class to use, table name and partition filter (optional).");
		inputOption.append(" Additional options for the input format can be "
				+ "specified as well.");
		inputOption.append(" You can set as many ").append(inputType)
				.append(" inputs as you like.");
		inputOption.append(" Example:\n");
		inputOption
				.append("\"My")
				.append(hiveToObjectClassName)
				.append(
	...
	/**
	 * Override this method to add more command-line options. You can process them
	 * by also overriding {@link #processMoreArguments(CommandLine)}.
	 * 
	 * @param options
	 *          Options
	 */
	...
	/**
	 * Override this method to process additional command-line arguments. You may
	 * want to declare additional options by also overriding
	...
	/**
	 * Return a copy of array from some position to the end, or empty array if
	 * startIndex is out of array size
	 * 
	 * @param array
	 *          Array to take a copy from
	 * @param startIndex
	 *          Starting position
